...
/*   Updated: 2024/03/12 12:14:05 by zel-harb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void full_str(t_img **read_m,char *argv)
{
    ...
    if (line != NULL) {
       ...
       (*read_m)->count_coll=ft_strchr_int((*read_m)->str,'C',(*read_m)->count);
...
{ 
    ...
    (*img)->img_exit[0]=mlx_xpm_file_to_image((*img)->mlx, "img/exit.xpm", &width, &height);
    (*img)->img_exit[1]=mlx_xpm_file_to_image((*img)->mlx, "img/51.xpm", &width, &height);
...
void print_window(t_img **img)
{
    ...
    while(i  < (*img)->count )
    {
        ...
        while(j < (*img)->len )
        {
            ...
            {
                if((*img)->count_coll==0)
                    mlx_put_image_to_window(((*img))->mlx,(*img)->mlx_win,(*img)->img_exit[1],j1, i1);
                else
                     mlx_put_image_to_window(((*img))->mlx,(*img)->mlx_win,(*img)->img_exit[0],j1, i1);
            }
            j++;
            j1+=50;
        }
